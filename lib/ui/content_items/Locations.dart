...
class Locations extends ExpandedStatefulWidget {
  Locations({Key? key, this.values, required this.action, this.inDropzone = false}) : super(key: key);

  @override
  String idBlock = "location";
  @override
...
class _LocationsState extends State<Locations> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      if (_searchController.text != "") {
        contents.addAll(await getContentByQueryAndByType(_actualPage, ContentType.PLACE, _searchController.text));
        contents.addAll(await getContentByQueryAndByType(_actualPage, ContentType.ORGANIZATION, _searchController.text));
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text("Ссылка на место",style: containerLabel,)
              ), 
                        ...
                        (BuildContext context, int index) {
                          return Row(
                            children: [
                              ...
                              Icon(Icons.place, color: Colors.purple.shade500),
